...
![版本](https://img.shields.io/badge/版本-v32.2.9-2ea44f)
...
<tr><td><strong>🔄 远程素材库</strong></td><td>SMB 同步,多台电脑共享同一个素材库</td></tr>
...
<table width="75%" align="center">
<thead>
<tr><th align="left" nowrap>对比项</th><th align="left"><strong>MuMu助手</strong>(本仓库)</th><th align="left">某牛</th><th align="left">某定设计</th></tr>
</thead>
<tbody>
...
<tr><td nowrap><strong>🔐 素材归属</strong></td><td><strong>100% 本地,数据完全自控</strong></td><td>云端,依赖厂商服务器</td><td>云端,依赖某定服务器</td></tr>
<tr><td nowrap><strong>🖥️ 多电脑同步</strong></td><td><strong>SMB 自建服务器</strong>,不经第三方</td><td>云端上传分享</td><td>云端同步(团队付费)</td></tr>
...
- 🆕 **v32.2.9** — 缩略图加载提速：预览图直接 file:// 秒开不再走慢速降级；缺缩略图的存量 psd 后台自动补生成（滚动不再占位割裂、延迟几秒）
- 🆕 **v32.2.8** — 插入细节：单层素材直接复制该层不套组（插入后保持图层形态）；多层素材才编组整体复制不闪烁
- 🆕 **v32.2.7** — 插入不再闪烁：素材 PSD 顶层先整理成单一组再整体复制，目标画布只重绘一次（多层散图层/混合结构也一样）
...
1. 本项目仅为开源的图形编辑器代码工具,项目源代码遵循 **MIT 协议** 开源分发,**仅提供编辑能力,不内置任何受版权保护的第三方模板、图片、字体、插画素材**。

2. 使用本软件进行设计时,由使用者自行上传、导入、引用的图片、字体、海报模板、图标、文案等一切第三方素材,其著作权、商标权、肖像权等相关法律责任全部由**使用者本人承担**。
使用者有义务确认所使用素材具备合法授权、满足商用或非商用使用条件;若因素材侵权引发纠纷,项目作者、贡献者不承担任何连带责任。

3. 本软件按“原样”提供,不提供任何形式的明示或暗示担保,不保证软件无缺陷、无故障、可满足特定业务目标。在法律允许最大范围内,项目版权持有人与贡献者,不对因使用或无法使用本软件所产生的任何直接、间接、附带或衍生损失承担赔偿责任。

4. 本项目仅为技术实现参考,产品功能思路参考市面上同类在线设计产品,**未复制、逆向、盗用任何竞品的源代码、UI资源、模板素材**。

5. 任何人 Fork、二次修改、私有化部署、二次分发本项目,同样需要遵守本免责条款;二次部署后的实例产生的侵权、纠纷,由部署运营方自行承担全部法律责任。

> 提示:如果你需要商用,请自行完成素材版权合规审核,不要直接抓取其他商业平台模板资源用于你的项目。
