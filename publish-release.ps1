...
try {
    ...
    $body = @{
        tag_name = $tag
        name = "MuMu助手 v$ver"
                        body = "## MuMu助手 v32.2.2`n`n### ✨ 本次新增`n- 远程同步大幅提速:检查素材先对比远程/本地根目录索引,只复制指纹(mtime+大小)不同的文件,素材无变化时秒完成`n- 重建索引大幅提速:改为增量指纹扫描,只处理新增/变化的素材,不再全量重读所有文件`n- 重建/扫描进度条显示已扫描分类数`n`n### ✨ 此前版本亮点`n- 排序随库走:分类顺序/素材手动排序固化进库内索引,U盘拷贝/换机/远程同步后排序一致`n- Windows↔macOS 跨平台拷贝:自动过滤旧路径残留条目并恢复排序`n- 远程同步增量更新、宽面板分类拖拽排序、多选保存自动编组`n- 语义化版本(主.次.修订):小改动只升小数点`n`n### 📦 部署`n- Windows:下载 zip 解压,双击【一键部署MuMu助手.bat】,重启 Photoshop 即可`n- Mac:下载 zip 解压,双击【一键部署MuMu助手.command】(无权限时终端执行 bash 一键部署MuMu助手.command),重启 Photoshop 即可"
        draft = $false
        prerelease = $false
    } | ConvertTo-Json
