...
try {
    ...
    $body = @{
        tag_name = $tag
        name = "MuMu助手 v$ver"
        body = "## MuMu助手 v$ver`n`n### ✨ 新增`n- 设置面板新增「检查更新」按钮:自动查询 GitHub Releases,发现新版本静默下载并覆盖更新,完成后提示重启 PS`n- 更新过程可视化:进度条实时显示下载百分比(流式下载)/解压/覆盖安装各阶段,并展示下载位置(完成后自动清理)`n- 网络不可达/访问超时给出明确中文提示(提示开启代理/VPN)`n`n### 🐛 修复`n- 修复切换素材库后素材全部变成「未分类」(索引分类 id 与重建后新分类失配,改为按文件夹名反查归类)`n- 修复同步完成后停在空分类看不到新素材(自动跳转到有素材的分类)`n- 全量重建提升至 120 秒超时,大库/慢机不再静默丢分类素材,失败有提示`n- 兜底救回与全量重建并发互踩`n- 修复 PS2024「检查素材」误报宿主脚本执行失败(远程同步超时提升至 120 秒,错误文案区分超时/脚本加载失败/网络不可达)`n`n### 📦 部署`n下载 zip 解压,双击【一键部署MuMu助手.bat】,重启 Photoshop 即可。"
        draft = $false
        prerelease = $false
    } | ConvertTo-Json
